...
<%@ attribute name="flag"   	   	type="java.lang.String" required="false" rtexprvalue="true" description="date picker option"%>
	...
	<c:set var="parsePattern" value="${empty parsePattern ? 'yyyy-MM-dd HH:mm':parsePattern}" />
	<c:set var="pattern" value="${empty pattern ? 'yyyy-MM-dd HH:mm':pattern}" />
	...
	<%-- 'MM' -> 'mm' --%>
	<c:set var="tmpPattern" value="${fn:replace(tmpPattern, 'MM','mm')}" />
	
	
	<c:set var="tmpPattern" value="${fn:split(tmpPattern, ' ')}" />
	
	<c:if test="${fn:length(tmpPattern)>1 }">
		<c:set var="dateFormat" value="${fn:trim(tmpPattern[0])}"/>
		<c:set var="timeFormat" value="${fn:trim(tmpPattern[1])}"/>
	</c:if>
	...
	<script language="javascript" type="text/javascript">
		$(function() {
			...
			$("${jqueryId}").datetimepicker(option);
		}); 
	</script>
